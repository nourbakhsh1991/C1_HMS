...
{
    // Command
    public class GetRolesCommand : IRequest<QueryResultsModel>
    {
        public QueryParamModel QueryParam { get; set; }
    }

    // Command Handler
    public class GetRolesCommandHandler : IRequestHandler<GetRolesCommand, QueryResultsModel>
    {
        ...
        public async Task<QueryResultsModel> Handle(GetRolesCommand request, CancellationToken cancellationToken)
        {
            var entities = _roleService.GetAsQueryable();
            // Filtering
            ...
            var result = entities.ToList()
                            .Aggregate(new List<MRole>(),
                                (list, current) =>
                                {
                                    var itm = current.IncludePermissions(_roleService);
                                    list.Add(itm.GetM());
                                    return list;
                                });
